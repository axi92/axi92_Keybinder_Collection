...
if(IsPlayerInAnyVehicle() == 1 || GetVehicleModelId() != 510 || GetVehicleModelId() != 509 || GetVehicleModelId() != 481 || GetVehicleModelId() != 509)
{
	handbremse := "[Taste:N]"
	if (IsVehicleEngineEnabled() == 1)
	{
		Random, Zufall, 1, 2, 3
		SendChat("/motor " Zufall)
	...
	else if (IsVehicleEngineEnabled() == 0)
	{
		if (IsVehicleLightEnabled() == 0)
		{
			SendChat("/licht")
		}
		Random, Zufall, 1, 2
...
OnPlayerExitVehicle()
{
	if (IsVehicleEngineEnabled() == 1)
	{
		Random, Zufall, 1, 2, 3
		SendChat("/motor " Zufall)
		if (IsVehicleLightEnabled() == 1 &&  IsPlayerDriver() == true )
		{
			SendChat("/licht")
